...
/*
here you create a token associted with governer contract, so that it can be used to vote,
(Note: once you transfer these tokens the voting power of those tokens are also moved to the
reciever of tokens),
here you can propose for a function to be executed by 'governor.propose' function, once
everyone voted and it matches or passes the criteria (0 - againest, 1 - for, 2 - abstain)
you can execute the proposal by 'governor.execute' and the required operation will take place.
*/
...
describe("MyGovernor", function () {
  ...
  describe("after proposing", () => {
    async function afterProposingFixture() {
      ...
      const { governor, token, otherAccount } = deployValues;

      const tx = await governor.propose(
        [token.address],
        [0],
        [token.interface.encodeFunctionData("mint", [otherAccount.address, parseEther("5000")])],
        "Give the pseudonym some tokens!"
      );
    ...
    describe("after voting", () => {
      async function afterVotingFixture() {
        ...
        const { governor, proposalId, owner, token } = proposingValues;
      ...
      it("should allow executing the proposal", async () => {
        const { governor, token, owner, otherAccount } = await loadFixture(afterVotingFixture);

        await governor.execute(
          [token.address],
          [0],
          [token.interface.encodeFunctionData("mint", [otherAccount.address, parseEther("5000")])],
          keccak256(toUtf8Bytes("Give the pseudonym some tokens!"))
        );

        const balance = await token.balanceOf(otherAccount.address);
        assert.equal(balance.toString(), parseEther("5000").toString());
      });
    });
  });
});